...
async def echo_server(loop: Loop | None) -> AsyncIterator[PyvoyServer]:
...
@pytest_asyncio.fixture(scope="module")
async def server(loop: Loop | None) -> AsyncIterator[PyvoyServer]:
    async with PyvoyServer(
...
@pytest.mark.asyncio
async def test_compression_disabled(loop: Loop | None) -> None:
    async with (
        PyvoyServer(
            "tests.apps.websockets.kitchensink",
            loop=loop,
...
@pytest.mark.asyncio
async def test_max_message_size(loop: Loop | None) -> None:
    async with PyvoyServer(
        "tests.apps.websockets.kitchensink",
        loop=loop,
        websockets=True,
...
@pytest.mark.asyncio
async def test_recv_after_disconnect(loop: Loop | None) -> None:
...
async def tls_server(
    tls_certs: tuple[trustme.CA, trustme.LeafCert, trustme.LeafCert], loop: Loop | None
