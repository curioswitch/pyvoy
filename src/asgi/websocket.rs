...
impl<EHF: EnvoyNetworkFilter> NetworkFilterConfig<EHF> for Config {
    fn new_network_filter(&self, _envoy: &mut EHF) -> Box<dyn NetworkFilter<EHF>> {
        Box::new(Filter {
...
impl<EHF: EnvoyNetworkFilter> NetworkFilter<EHF> for Filter {
    fn on_destroy(&mut self, _envoy_filter: &mut EHF) {
